...
__global__ void intensityDetect(
    ...
    unsigned xIndex = blockIdx.y * blockDim.y + threadIdx.y;
    unsigned zIndex = blockIdx.z;
...
__global__ void reducedSum(
    ...
    const unsigned pixId = idx * imgSz.y + idy;
    const float2 this_P = probe[pixId];

    float2 gradP = make_float2(0.0f, 0.0f);    // accumulate probe gradient per thread
    float *dldw1_linear = reinterpret_cast<float *>(dldw1);

    for (unsigned idz = 0; idz < imgSz.z; ++idz) {
        unsigned pageId = idz * imgSz.x * imgSz.y;

        const float2 this_W = latentW[pixId + pageId];
        const float2 this_X = forward[pixId + pageId];

        // dldw2??onj(W) * X
        gradP.x += this_X.x * this_W.x + this_X.y * this_W.y;
        gradP.y += this_X.x * this_W.y - this_X.y * this_W.x;

        // dldw1??onj(P) * X
        const int2 pos = position[idz];
        unsigned pixId_large = (pos.x + idx - 1) * imgHsz.y + (pos.y + idy - 1);

        float2 gradW;
        gradW.x = this_X.x * this_P.x + this_X.y * this_P.y;
        gradW.y = this_X.x * this_P.y - this_X.y * this_P.x;

        atomicAdd(dldw1_linear + 2 * pixId_large    , gradW.x);
        atomicAdd(dldw1_linear + 2 * pixId_large + 1, gradW.y);
    }
    dldw2[pixId] = gradP; // probe gradient can be written directly since no overlap
    // float *dldw2_linear = reinterpret_cast<float *>(dldw2);
    // atomicAdd(dldw2_linear + 2 * pixId    , gradP.x);
    // atomicAdd(dldw2_linear + 2 * pixId + 1, gradP.y);
}
