...
__global__ void fullyfused_shiftNprop(
    ...
    unsigned pix_id = idx * imgSzH.y + idy;
    unsigned page_id = idz * (imgSzH.x * imgSzH.y);
    ...
    creal32_t this_w1 = wavefront1[pix_id];
    creal32_t this_x;
...
__global__ void fullyfused_shiftNprop_Bwd(
    ...
    unsigned pageSz = imgSzH.x * imgSzH.y;
    ...
    creal32_t this_out = outpus[pix_id + page_id];
    creal32_t out;
...
__global__ void pixelwiseProduct_conj(
    ...
    if (inside) {
        ...
        creal32_t this_X = X[pix_id + page_id];
        creal32_t this_Y = Y[pix_id];
        creal32_t this_Z;    
...
__global__ void reducedSum_Bwd(
    ...
    for(unsigned idz = 0; idz < imgSz.z; ++idz){
        ...
        creal32_t this_fwd = forward[pix_bc];
        creal32_t this_bwdrec = bwd_record[pix_bc];
        creal32_t this_fwdrec = fwd_record[pix_bc];
