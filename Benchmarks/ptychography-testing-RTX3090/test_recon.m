...
pix = 512;                     % Low-res measurement size (detector patch size)
...
sample = 900;                      % Number of diffraction measurements (scan positions)
...
batchSize = 64;                     % Mini-batch size (number of positions per step)
...
adfoo = adPtychoX_base("enable_cuda", true,...
                        ...
                        "probe_size",  [pix,pix],...
                        "sample_size", [pix * mag,pix * mag]); % Differentiable ptychography operator (custom)
...
for epoch = 1:600
    ...
    while ~isempty(listBlock)
        ...
        if mod(iteration, 5) == 0
            ...
            if frame == 1
                imwrite(frame_gray, 'test.gif', 'gif', ...
                        'LoopCount', 65535, ...
                        'DelayTime', 0.03);
