 ...
 *   X(k, idz) = wavefront1(k) * exp(+j * 2? * (kx*dx + ky*dy)) * prop(k)
 ...
 *   If forward does: y = (x * exp(+j?)) * prop
 *   Then adjoint should do: x_grad = (y_grad * conj(prop)) * exp(-j?)
 *   plus possible FFT/IFFT scaling depending on conventions.
 *
 * What this kernel currently implements
 *   - Multiplies by exp(-j?) (implemented as combination of re/im)
...
__global__ void fullyfused_shiftNprop_Bwd(
    ...
    // Apply exp(-j?) to this_out (note the sign pattern)
...
__global__ void fullyfused_ReducedSum(
    ...
    for (unsigned idz = 0; idz < imgSzH.z; ++idz){
        ...
        // Phase argument: (2?*kx)*dx + (2?*ky)*dy
...
__global__ void deconvCodedSurf(
    ...
    creal32_t this_dldw2 = {0.0f, 0.0f};
    const unsigned pageSz = imgSz.x * imgSz.y;
    ...
    for(unsigned idz = 0; idz < imgSz.z; ++idz){
        ...
        this_dldw2.re += this_X.re * this_C.re + this_X.im * this_C.im;
        this_dldw2.im += this_X.re * this_C.im - this_X.im * this_C.re;
    }
    dldw2[pix_id] = this_dldw2;
}
...
__global__ void fullyfused_DownSample_Fwd(
    ...
    const unsigned pix_id     = idx * imgSz.y + idy;
    const unsigned page_id    = idz * (imgSz.x * imgSz.y);          // small-page offset
    const unsigned page_large = page_id * ds * ds;                  // large-page offset (assumed)
...
__global__ void fullyfused_DownSample_Bwd(
    ...
    const unsigned pix_id     = idx * imgSz.y + idy;
    const unsigned page_id    = idz * (imgSz.x * imgSz.y);
    const unsigned page_large = page_id * ds * ds;
