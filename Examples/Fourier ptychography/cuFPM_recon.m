...
optimizer_w1 = optimizers.Adam(0.9,0.999,1e-15);
optimizer_w2 = optimizers.Adam(0.9,0.999,1e-15);

lr = 0.01;

cuFPM_foo = adPtychoX_fpm( ...
    "batch_size",batchSize, ...
    "high_Res",pratio * size(imRaw,1), ...
    "low_Res",size(imRaw,1) ...
    ); % init differentiable function

% cuFPM_foo_GPU = adPtychoX_fpm( ...
%     "batch_size",batchSize, ...
%     "enable_cuda",false, ...
%     "large_image_size",pratio * size(imRaw,1), ...
%     "small_image_size",size(imRaw,1) ...
%     ); % init differentiable function
...
while epoch < numEpochs
    ...
    while last < size(imRaw,3)
        ...
        if size(batchIdx,2) < batchSize
            ...
            temp_empty = randperm(size(imRaw,3));
            batchIdx = [batchIdx,temp_empty(1:empty)];
        end

        shifted_fq =  (-size(imRaw,1) * downsam + size(imRaw,1) * pratio)/2;
        ledIdx =  int32(gpuArray(pos(:,batchIdx))) + shifted_fq;



        % pred_Y = cuFPM_foo.forward(wave1, ...
        %          wave2, ...
        %          imRaw(:,:,batchIdx), ...
        %          single(ledIdx), ...
        %          pratio);
        % 
        % 
        % pred_Y_GPU = cuFPM_foo_GPU.forward(wave1, ...
        %          wave2, ...
        %          imRaw(:,:,batchIdx), ...
        %          single(ledIdx), ...
        %          pratio);
        ...
        [loss,dldw1,dldw2] = dlfeval(@autodiff_fpm,...
                                           ...
                                           ledIdx, ...
                                           pratio);
        % diff = sum((pred_Y - pred_Y_GPU).^2,'all')
        first = last + 1;
...
% loss = srcs.l2_loss(pred_Y, obse_Y, 'isotropic');
% loss_tv = srcs.fd_loss(wave1, 0, 'anisotropic');
loss = srcs.fd_loss(pred_Y, obse_Y, 'anisotropic');
